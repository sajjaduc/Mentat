...
import { syncTicketProjectionSync } from '../records/projection';
...
export function createTicketSync(
  ...
  // Project the ticket into the universal Record + WorkflowItem model (ADR-0021).
  syncTicketProjectionSync(tx, actor.workspaceId, ticketId);
...
export function updateTicketSync(
  ...
  syncTicketProjectionSync(tx, actor.workspaceId, ticket.id);
  return ticket;
}
