...
board. **Tickets** move through user-defined **States**; a state can wait for a
human, invoke an agent, require an approval, or run deterministic work. Mentat
provides the infrastructure agents repeatedly need — HTTP integrations, persistent
state, structured data, cache, secrets, webhooks, cron, approvals, retries,
artifacts, audit history, providers and models — so you build workflows instead of
plumbing.
...
4. Bind the agent to a state (**Workflow → Configuration → States**) and move a
   ticket into it. Watch it happen on the ticket's **Agent Work** tab.
...
| Area | Highlights |
| --- | --- |
| Workflows | States (human / agent / system / terminal), transitions, human gates, per-state retries, timeouts, failure routing, WIP limits, five seed templates |
| Tickets | Per-workspace keys, typed fields, notes with revisions, labels, relationships, artifacts, cross-workflow transfer with field mapping, one coherent history |
...
| Analytics | Dashboards built from the same filter AST as ticket lists, with funnels and time-in-state derived from recorded history |
...
- **Human gates are not advisory.** An agent cannot move a ticket out of a gated
