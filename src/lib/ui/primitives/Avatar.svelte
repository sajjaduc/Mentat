<script lang="ts">
  ...
  interface Props { id: string; name?: string | null; size?: 'xs' | 'sm' | 'md' | 'lg'; class?: string }
  let { id, name, size = 'sm', class: className = '' }: Props = $props();
  const sizes = { xs: 'h-5 w-5 text-[9px]', sm: 'h-6 w-6 text-[10px]', md: 'h-8 w-8 text-xs', lg: 'h-10 w-10 text-sm' } as const;
