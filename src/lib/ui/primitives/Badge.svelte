<script lang="ts">
  ...
  let { tone = 'neutral', size = 'sm', dot = false, class: className = '', children }: Props = $props();

  const tones: Record<string, string> = {
    neutral: 'bg-[var(--color-surface-muted)] text-[var(--color-ink-muted)] border-[var(--color-border-subtle)]',
    accent: 'bg-[var(--color-accent-soft)] text-[var(--color-accent-ink)] border-transparent',
    positive: 'bg-[color-mix(in_oklch,var(--color-positive)_18%,transparent)] text-[var(--color-positive)] border-transparent',
    caution: 'bg-[color-mix(in_oklch,var(--color-caution)_22%,transparent)] text-[color-mix(in_oklch,var(--color-caution)_70%,var(--color-ink))] border-transparent',
    danger: 'bg-[color-mix(in_oklch,var(--color-danger)_16%,transparent)] text-[var(--color-danger)] border-transparent',
