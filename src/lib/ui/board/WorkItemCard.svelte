...
interface Props {
  ...
  onDragEnd: () => void;
  /** Singular noun from the Workflow's Object Type. */
  nounSingular?: string;
}
...
<div
  role="listitem"
  aria-label="{row.workItem.record.key ?? ''} {row.workItem.record.displayName}"
...
>
  <div class="flex items-start gap-1.5">
    <button
      ...
      onclick={onOpen}
    >
      <span class="font-mono text-[10px] text-[var(--color-ink-subtle)]">{row.workItem.record.key}</span>
      <span class="mt-0.5 block text-sm leading-snug font-medium">{row.workItem.record.displayName}</span>
    </button>
  ...
  <div class="mt-2 flex flex-wrap items-center gap-1.5">
    {#if row.workItem.priority !== 'none'}
      <Badge tone={priorityTone(row.workItem.priority)} dot={true}
    ...
    {:else if row.workItem.ownerTeamId}
      <Badge tone="muted">{teams.find((team) => team.id === row.workItem.ownerTeamId)?.name ?? 'Team'}</Badge>
    {/if}
    {#if pending}
      <span
