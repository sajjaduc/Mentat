 ...
 * answer. The feed re-reads itself when a live event arrives for this ticket.
 */
              ...
              <li class="flex gap-2 border-l-2 border-[var(--color-border-subtle)] pl-3">
                <div class="min-w-0 flex-1">
                  ...
                  <div class="mt-0.5 flex gap-2 text-[10px] text-[var(--color-ink-subtle)]">
                    {#if event.ticketId}
                      <a class="underline decoration-dotted" href={`/tickets/${event.ticketId}`}>ticket</a>
                    {/if}
                    {#if event.runId}
