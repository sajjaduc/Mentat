...
import RunTimeline from '$ui/work-item/RunTimeline.svelte';
...
let { workflowItemId, currentStateId, transitionNames, actions }: Props = $props();
...
async function loadRuns() {
  ...
  try {
    const response = await api.get<{ runs: AgentRun[] }>(
      `/api/workflow-items/${workflowItemId}/runs`
    );
    runs = response.runs;
...
function handleEvent(event: WorkEvent) {
  ...
  if (event.type === 'state.transition' || event.type === 'workItem.updated') {
...
<div class="space-y-3 p-4" data-testid="agent-work-tab">
  <div class="flex flex-wrap items-center gap-2">
    ...
    <span class="ml-auto flex items-center gap-2">
      <a
        ...
        href={`/approvals?workflowItemId=${workflowItemId}`}>Run approvals</a
      >
  ...
  {:else if runs.length === 0}
    <EmptyState
      title="No runs yet"
      description="Runs appear here when the work item enters an agent state or when you press Run now."
    >
