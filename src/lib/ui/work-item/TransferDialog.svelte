 ...
 *
 * Moving a workItem between workflows is a controlled operation, so the dialog shows
...
let { open, workflowItemId, currentWorkflowId, onclose, onTransfer }: Props = $props();
...
async function chooseTarget(id: string) {
  ...
  loading = true;
  try {
    const response = await api.post<TransferPreview>(
      `/api/workflow-items/${workflowItemId}/transfer-preview`,
      {
        targetWorkflowId: id
      }
    );
...
<Modal
  {open}
  title="Transfer workItem"
  description="Move this work item to another workflow. The Record, notes, artifacts and history are preserved."
  width="40rem"
  onclose={onclose}
>
  <div class="space-y-4">
    <Select
      ...
      options={workflows.map((workflow) => ({
        value: workflow.id,
        label: `${workflow.name} (${workflow.itemCount} items)`
      }))}
