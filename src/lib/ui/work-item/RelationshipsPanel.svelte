 ...
 *
 * Links are created from a workItem search rather than by pasting an id, so the
 * person picking a parent or duplicate sees the workItem they are choosing. Removal
...
import type { WorkItemRelationshipView } from '$ui/work/types';
...
interface Props {
   ...
   */
  onAdd: (toWorkItemId: string, type: string, note?: string) => Promise<unknown>;
  onRemove: (relationshipId: string) => Promise<void>;
  onOpenWorkItem: (workflowItemId: string) => void;
}

let { workflowItemId, relationships, onAdd, onRemove, onOpenWorkItem }: Props = $props();
...
async function search() {
  ...
  try {
    ...
    results = response.results.filter((result) => result.id !== workflowItemId);
  } catch (failure) {
...
async function link(targetId: string) {
  ...
  if (applied) {
    ...
    pushToast({ tone: 'error', title: 'Could not link the workItem' });
  }
}
</script>
...
<section class="space-y-2">
  ...
  {#if linking}
    <div class="space-y-2 rounded-[var(--radius-md)] border border-[var(--color-border-subtle)] p-2">
      ...
      {:else if query.trim().length >= 2}
        <p class="text-xs text-[var(--color-ink-subtle)]">No matching work items.</p>
      {/if}
    </div>
  {/if}
  ...
  {#if relationships.length === 0}
    <p class="text-xs text-[var(--color-ink-subtle)]">
      No links yet. Relate work items to show dependencies, duplicates or decomposition.
    </p>
  {:else}
    <ul class="space-y-1">
      {#each relationships as relationship (relationship.id)}
        ...
        >
          ...
          <button
            ...
            onclick={() => onOpenWorkItem(relationship.workItem.id)}
          >
            <span class="font-mono text-[10px] text-[var(--color-ink-subtle)]"
              >{relationship.workItem.key}</span
            >
