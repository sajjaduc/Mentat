 ...
 * controlled by the parent (the server's ticket endpoint cannot sort reliably, so
...
let {
  ...
  members = [],
  teams = [],
  emptyTitle = 'No tickets here yet',
  emptyDescription = 'Tickets appear here as soon as work is created in this view.'
}: Props = $props();

function fieldValue(row: TicketListRow, key: string): unknown {
      ...
      <tbody>
        {#each rows as row (row.ticket.id)}
          <tr
            ...
            onclick={() => onRowClick(row.ticket.id)}
          >
            {#each columns as column (column.key)}
              <td class="px-3 py-2 align-middle text-sm">
                ...
                {:else if column.key === 'title'}
                  <div class="flex min-w-0 items-center gap-2">
                    <span class="truncate font-medium">{row.ticket.title}</span>
                ...
                {:else if column.key === 'priority'}
                  <Badge tone={priorityTone(row.ticket.priority)}
                ...
                {:else if column.key === 'owner'}
                  ...
                  {:else if row.ticket.ownerTeamId}
                    <span class="text-xs text-[var(--color-ink-muted)]">{teams.find(
                        (team) => team.id === row.ticket.ownerTeamId
                      )?.name ?? 'Team'}</span
                    >
                  {:else}
                ...
                {:else if column.key === 'updated'}
                  <span class="text-xs text-[var(--color-ink-muted)]"
                    >{formatRelative(row.ticket.updatedAt)}</span
                  >
                {:else if column.key === 'due'}
                  <span class="text-xs text-[var(--color-ink-muted)]"
                    >{formatDate(row.ticket.dueAt)}</span
                  >
                {:else if column.key === 'age'}
                  <span class="text-xs text-[var(--color-ink-muted)]"
                    >{ageInState(row.ticket.enteredStateAt)}</span
                  >
